...
<!-- #raw -->
p.show(figsize=16, aspect_ratio=1, xmin=128, xmax=256, ymin=64, ymax=128)
<!-- #endraw -->

```sage
dd = long_diagonals(_g, xmax=1024, ymax=256)
```
...
for y in ylist[:20]:
    try:
        ...
        x = '-'
    print("{:12s} {:12s} {}".format(str(y), str(x), len(ypoints[y])))
```
...
G = Graphics()
for yy in ylist:
    G += point2d(((x, y) for (x, y) in dd if y == yy), hue=yy/2, size=2)
```
...
for yy in ylist:
    ...
    p = point2d((x for (x, y) in dd if y == yy), hue=yy/2, xmin=0, xmax=R/2.5, ymin=0, ymax=R/2.5, size=4)
    GG.append(c + p + l)
```
...
```sage
for y in ylist:
    print('{:15s} = {:54s}{}'.format(str(y), str(y.factor()), ' (prime)' if y.is_prime() else ''))
```
...
for yy in ylist:
    ...
    p = point2d((x for (x, y) in dd if y == yy), hue=yy/2, xmin=0, xmax=R/2, ymin=0, ymax=R/2, size=4)
    GGG.append(c + p + l)
```
...
<!-- #raw -->
ylist = sorted(set(y for (x, y) in dd if x0 < x < x1))
<!-- #endraw -->
...
```sage
point2d(horiz_count.items(), size=4)
```

```sage
point2d(((y, Integer(n)/(_a*y)) for y, n in horiz_count.items()), size=4)
```
...
```sage
prime_statistics(density_local[7])
```
...
```sage
m = prod(cone_contraction[j] for j in w)
```

```sage
p = point2d(m*v for v in (_V((1, 0)), _V((0, 1))))
```

```sage
l = line2d(m*v for v in (_V((1, 0)), _V((0, 0)), _V((0, 1))))
```
...
```sage
for y, xx in density_7_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```

```sage
for y, xx in density_6_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```

```sage
for y, xx in density_5_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```
...
```sage
for g in (plot_vectors_y_xx(y, xx) for y, xx in density_7_xy.iteritems()):
    g.show(xmin=-80, ymin=-55, xmax=80, ymax=55, aspect_ratio=0.5, figsize=6)
```

```sage
for xy in density_7_xy.iteritems(): print xy
```
...
```sage
for g in (plot_vectors_y_xx(y, xx) for y, xx in density_6_xy.iteritems()):
    g.show(xmin=-80, ymin=-55, xmax=80, ymax=55, aspect_ratio=0.5, figsize=6)
```

```sage
for g in (plot_vectors_y_xx(y, xx) for y, xx in density_5_xy.iteritems()):
    g.show(xmin=-80, ymin=-55, xmax=80, ymax=55, aspect_ratio=0.5, figsize=6)
```
...
orbb_ylist = sorted(orbb_ypoints.iterkeys())
```
...
```sage
for y, xx in orbb_density_10_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```

```sage
for y, xx in orbb_density_9_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```

```sage
for g in (plot_vectors_y_xx(y, xx) for y, xx in orbb_density_10_xy.iteritems()):
    g.show(aspect_ratio=0.5, figsize=6)
```

```sage
sum((plot_vectors_y_xx(y, xx) for y, xx in orbb_density_10_xy.iteritems()), Graphics()).show(aspect_ratio=0.5, figsize=6)
```
...
```sage
for yy, xx in orbb_density_10_xy.iteritems():
    G = point2d(xx)
    ...
    for x in xx:
        ...
        G += text('$y = {} + {} \phi$'.format(yy[0], yy[1]), (amin, bmin + bpad), horizontal_alignment='left')
...
```sage
for yy, xx in orbb_density_9_xy.iteritems():
    G = point2d(xx)
    amin, amax = min(x[0] for x in xx), max(x[0] for x in xx)
    apad = (amax - amin)/10
    bmin, bmax = min(x[1] for x in xx), max(x[1] for x in xx)
...
orbc_ylist = sorted(orbc_ypoints.iterkeys())
```
...
```sage
for yy, xx in orbc_density_18_xy.iteritems():
    G = point2d(xx)
    amin, amax = min(x[0] for x in xx), max(x[0] for x in xx)
    apad = (amax - amin)/10
    bmin, bmax = min(x[1] for x in xx), max(x[1] for x in xx)
...
```sage
for yy, xx in orbc_density_15_xy.iteritems():
    G = point2d(xx)
    amin, amax = min(x[0] for x in xx), max(x[0] for x in xx)
    apad = (amax - amin)/10
    bmin, bmax = min(x[1] for x in xx), max(x[1] for x in xx)
...
```sage
for yy, xx in orbc_density_14_xy.iteritems():
    G = point2d(xx)
    amin, amax = min(x[0] for x in xx), max(x[0] for x in xx)
    apad = (amax - amin)/10
    bmin, bmax = min(x[1] for x in xx), max(x[1] for x in xx)
...
```sage
for yy, xx in orbc_density_13_xy.iteritems():
    G = point2d(xx)
    amin, amax = min(x[0] for x in xx), max(x[0] for x in xx)
    apad = (amax - amin)/10
    bmin, bmax = min(x[1] for x in xx), max(x[1] for x in xx)
...
```sage
for y, xx in orbc_density_18_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```

```sage
for y, xx in orbc_density_15_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```

```sage
for y, xx in orbc_density_14_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```
...
orbd_ylist = sorted(orbd_ypoints.iterkeys())
```
...
orbe_ylist = sorted(orbe_ypoints.iterkeys())
```
...
orbf_ylist = sorted(orbf_ypoints.iterkeys())
```
...
```sage
nmaxf = max(orbf_density_local.keys())
```
...
```sage
for y, xx in orbf_density_max_xy.iteritems():
    vectors_and_words(y, xx)
    print ""
```
...
```sage
for y, xx in horiz_fund_domain.iteritems():
    d = defaultdict(int)
    for x in xx:
        d[floor(x)] += 1
    horiz_by_floor[y] = d
...
```sage
point2d(horiz_max_with_same_floor.iteritems(), size=4)
```

```sage
for y, n in horiz_max_with_same_floor.iteritems():
    if n > 4:
...
```sage
point2d((y[1]/y[0], n) for y, n in local_density.iteritems() if y[0])
```
...
line2d([(_a, 0), (_a, 7)], color='red') + point2d(
    (y[1]/y[0], n) for y, n in horiz_max_with_same_floor.iteritems() if y[0] and 1.61 < y[1]/y[0] < 1.63)
```
...
```sage
G = point2d(yfloorn.iteritems(), size=8, ymin=0)
G += plot(lambda x: 0.15*exp(0.5*ln(x))*ln(x), (1, 1000), color='red')
G
```

```sage
G = point2d(((n, ln(RDF(k))/ln(RDF(n))) for (n, k) in yfloorn.iteritems() if y > 1 and n > 1), size=8, ymin=0)
G += line2d(((0, 0.5), (1000, 0.5)), color='red')
G
```

```sage
point2d(yfloorn.iteritems(), size=8, ymin=0, xmax=300, ymax=20)
```

```sage
point2d((y, len(xx)) for y, xx in ypoints.iteritems())
```

```sage
ypoints_sorted = []
for y in sorted(ypoints.iterkeys())
